...
import com.tompy.entity.encounter.internal.EncounterImpl;
...
public class EntityServiceImpl implements EntityService {
    ...
    @Override public ItemBuilder createItemBuilder() {
        entityKey++;
    ...
    @Override public EntityService add(Entity entity, Attribute attribute) {
        attributeManagers.get(entity.getKey()).add(attribute);
        return this;
    }

    @Override public EntityService add(Entity entity, Attribute attribute, Integer value) {
        attributeManagers.get(entity.getKey()).add(attribute, value);
        return this;
    }

    @Override public void remove(Entity entity, Attribute attribute) {
        attributeManagers.get(entity.getKey()).remove(attribute);
    }

    @Override public void reset(Entity entity, Attribute attribute) {
        attributeManagers.get(entity.getKey()).reset(attribute);
    }

    @Override public boolean is(Entity entity, Attribute attribute) {
        return attributeManagers.get(entity.getKey()).is(attribute);
    }

    @Override public OptionalInt valueFor(Entity entity, Attribute attribute) {
        return attributeManagers.get(entity.getKey()).getValue(attribute);
    }

    @Override public Entity getEntityByName(String name) {
        return entityMap.get(name);
    }

    @Override public Item getItemByDescription(String description) {
    ...
    @Override public Feature getFeatureByDescription(String description) {
        return null;
    }

    @Override public Area getAreaByName(String name) {
        return areas.stream().filter(a -> name.equals(a.getName())).findAny().get();
    }

    @Override public String getAttributeDescription(Entity key, Attribute attribute) {
        if (is(key, attribute)) {
    ...
    @Override public void addAttributeDoesApply(Entity key, Attribute attribute, String text) {
        attributeManagers.get(key).addApply(attribute, text);
    }

    @Override public void addAttributeDoesNotApply(Entity key, Attribute attribute, String text) {
        attributeManagers.get(key).addDesNotApply(attribute, text);
    }

    @Override public String getAttributeApplicationText(Entity key, Attribute attribute) {
        return attributeManagers.get(key).getApplication(attribute, is(key, attribute));
    }

    @Override public EntityService add(Entity entity, EventType type, Event event) {
        eventManagers.get(entity.getKey()).add(type, event);
        return this;
    }

    @Override public void remove(Entity entity, EventType type, Event event) {
        eventManagers.get(entity.getKey()).remove(type, event);
    }

    @Override public List<Event> get(Entity entity, EventType type) {
        return eventManagers.get(entity.getKey()).getAllOfType(type);
    }

    @Override public List<Response> handle(Entity entity, EventType type, Player player, Adventure adventure) {
    ...
    @Override public FeatureBuilder createFeatureBuilder() {
        entityKey++;
        attributeManagers.put(entityKey, attributeManagerFactory.create());
        eventManagers.put(entityKey, eventManagerFactory.create());
    ...
    @Override public void addFeature(Feature feature) {
        if (feature != null) {
    ...
    @Override public AreaBuilder createAreaBuilder() {
        entityKey++;
        attributeManagers.put(entityKey, attributeManagerFactory.create());
        eventManagers.put(entityKey, eventManagerFactory.create());
    ...
    @Override public void addArea(Area area) {
        if (area != null) {
            areas.add(area);
    ...
    @Override public EventBuilder createEventBuilder() {
        entityKey++;
        attributeManagers.put(entityKey, attributeManagerFactory.create());
        eventManagers.put(entityKey, eventManagerFactory.create());
